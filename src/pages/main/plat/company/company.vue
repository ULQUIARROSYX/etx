<template>
  <div style="width: 700px; max-width: 100%">
    ...
    <div class="row justify-between items-center">
      ...
      <q-btn color="secondary" icon="add_box" glossy label="添加新公司" @click="companyMsg($route.query.proId)" />
    </div>
    <q-table class="q-mt-md q-mb-lg" :data="companyData.data" :columns="companyData.columns" :pagination.sync="pagination" row-key="name">
      <!--表格主体-->
      <q-tr slot="body" slot-scope="props" :props="props" @click.native="companyMsg(props.row.companyId)" class="cursor-pointer">
        <q-td key="companyId" :props="props">{{ props.row.companyId }}</q-td>
        ...
        <q-td key="more" :props="props" class="cursor-pointer">
          <q-btn size="sm" rounded push color="brown-5" @click.stop="companyOp(props.row.companyId)">人员查看</q-btn>
        </q-td>
      </q-tr>
    </q-table>
  </div>
